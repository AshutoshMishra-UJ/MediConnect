...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    // Keep the original animation when WebGL is available.
    let renderer;
    try {
      ...
      console.warn('WebGL is unavailable; using the CSS globe animation.', error);
  ...
  return (
    <section ref={sectionRef} id="home" className="relative h-screen flex items-center overflow-hidden">
      ...
      {webglUnavailable && (
        <div className="absolute right-[8%] top-1/2 z-0 h-72 w-72 -translate-y-1/2 rounded-full border-2 border-blue-300/60 opacity-80 shadow-[0_0_80px_rgba(37,99,235,0.45)]" aria-hidden="true">
          <div className="absolute inset-0 rounded-full" style={{
            background: 'radial-gradient(circle at 35% 30%, rgba(96,165,250,0.7), rgba(0,86,179,0.35) 45%, transparent 70%)',
            animation: 'hero-globe-pulse 6s ease-in-out infinite'
          }} />
          <div className="absolute inset-[-1px] rounded-full" style={{
            background: 'repeating-radial-gradient(ellipse at center, transparent 0 18px, rgba(125,211,252,0.45) 19px 20px), repeating-linear-gradient(90deg, transparent 0 22px, rgba(125,211,252,0.35) 23px 24px)',
            animation: 'hero-globe-spin 18s linear infinite'
          }} />
          <style>{`@keyframes hero-globe-spin { from { transform: rotateY(0deg) rotateZ(0deg); } to { transform: rotateY(360deg) rotateZ(12deg); } } @keyframes hero-globe-pulse { 0%, 100% { transform: scale(0.96); opacity: 0.7; } 50% { transform: scale(1.04); opacity: 1; } }`}</style>
        </div>
      )}
      ...
      <div className="container mx-auto px-6 z-10 relative">
        <div className="max-w-3xl">
          ...
          >
            <motion.span
              className={`inline-block ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'
          ...
          <motion.p
            ...
            className={`text-xl ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'
